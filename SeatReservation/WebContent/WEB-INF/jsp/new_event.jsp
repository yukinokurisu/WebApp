...
<%
	...
	if (shop_name == null) {
		shop_name = shops.get(0).getShop_name();
	}
	//初期値の受け取り（詳細）
		...
		<div class="collapse navbar-collapse" id="navbarNav3">
			<ul class="navbar-nav">
				<li class="nav-item active"><a class="nav-link"
					href="./TopServlet">Top <span class="sr-only">(current)</span></a>
				</li>
		...
		<div class="col-12">
			<form action="./NewEventServlet" method="post">
				<div class="form-group">
					<label for="text1">イベント名<font color="RED">(必須)</font>:
					</label> <input name="title" type="text" class="form-control"
						value="<%=title%>" required>
				</div>
				<div class="start_at">
					<label for="text1">日時 (半角数字で入力 例:1999-02-23)<font
						color="RED">(必須)</font>:
					</label> <input name="start_at" type="date" class="form-control"
						<%if (start_at_string != null) {%> value=<%=start_at_string%>
						<%}%> required>
				</div>
				<%
				...
				%>
				<div class="form-group">
					<label for="text1">定員<font color="RED">(必須)</font>:
					</label> <input name="capacity" type="text" class="form-control"
						<%if (capacity != null && request.getAttribute("capacityError") == null) {%>
						value=<%=Integer.parseInt(capacity)%> <%}%> required>
				</div>
				<%
				...
				%>
				<div class="form-group">
					<label for="text1">開催店名<font color="RED">(必須)</font>:
					</label>
					...
					<select name="shop_name" id="shop_name">
						<%
							for (Shop shop : shops) {
						%>
						<option value="<%=shop.getShop_name()%>"
							<%if (shop_name.equals(shop.getShop_name())) {%> selected <%}%>>
							<%=shop.getShop_name()%>
						</option>
						<%
							}
				...
				<div class="form-group">
					<label for="text1">詳細:</label> <input name="description"
						type="text" class="form-control" value="<%=description%>">
				</div>
				<div class="form-group">
